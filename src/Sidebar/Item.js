...
export default function Item({lection}) {
    ...
    // TODO link to the lection on the double click
    // TODO link to the test on click

    return (
        <li key={lection.id}>
            {/* eslint-disable-next-line no-script-url */}
            <button onClick={() => processTests()} type="button" className="flex items-center w-full p-2 text-base text-gray-900 transition duration-75 rounded-lg group hover:bg-gray-100 dark:text-white dark:hover:bg-gray-700" aria-controls={`tests-for-${lection.id}`} data-collapse-toggle={`tests-for-${lection.id}`}>
            ...
            <ul id={`tests-for-${lection.id}`} className={`${showTests ? '' : 'hidden'} py-2 space-y-2 ml-3`}>
                {
                    tests.map(test => (
                        <li key={test.id} className="hover:cursor-pointer">
